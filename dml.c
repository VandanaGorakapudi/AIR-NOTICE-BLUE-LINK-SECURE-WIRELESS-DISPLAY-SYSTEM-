...
//delay is used for user visibility
	for(l=0;l<delay;l++)	
	{
		for(j=0;j<8;j++)	//used for selecting the row by row
		{
			...
			SIPO_74LS164(SIN1,CP1,font[ch-32][j]);
...
void display_string(unsigned char *string)
{
	...
	for(l=0;l<500;l++)	
	{
		...
		{
		    WRITEBYTE(IOPIN0,ROW,0x00);
			SIPO_74LS164(SIN1,CP1,font[string[0]-32][j]);
			SIPO_74LS164(SIN2,CP2,font[string[1]-32][j]);
			SIPO_74LS164(SIN3,CP3,font[string[2]-32][j]);
			SIPO_74LS164(SIN4,CP4,font[string[3]-32][j]);
			WRITEBYTE(IOPIN0,ROW,(1<<j));
			delay_us(100);
		 }
		 if(r_flag==1)
		 break;
